        ...
        <div>
          ...
          <ul className="mt-3 space-y-2.5 text-sm text-[var(--color-ink-soft)]">
            ...
            <li className="flex items-center gap-2">
              ...
              <a className="hover:text-[var(--color-sage-dark)]" href="mailto:kontakt@klarhverdag.dk">kontakt@klarhverdag.dk</a>
            </li>
            <li className="flex items-center gap-2">
              <MapPin className="h-4 w-4 shrink-0 text-[var(--color-sage-dark)]" aria-hidden="true" />
              <span>Nørrebrogade 22, 2200 København N</span>
            </li>
          </ul>
        </div>
      </div>
      <div className="border-t border-[var(--color-line)] py-5 text-center text-xs text-[var(--color-ink-soft)]">
        © {new Date().getFullYear()} Klar Hverdag Ergoterapi · CVR 12345678 · Illustrationer af{" "}
        <a className="hover:text-[var(--color-sage-dark)]" href="https://undraw.co" target="_blank" rel="noreferrer">
          unDraw
        </a>
      </div>
    </footer>
